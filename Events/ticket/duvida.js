...
client.on("interactionCreate", async interaction => {
    ...
    if (interaction.isButton()) {
        if (interaction.customId === "duvidas") {
            if (interaction.guild.channels.cache.find(c => c.name === `dúvida-${member.id}`)) {
                    let c = interaction.guild.channels.cache.find(c => c.name === `dúvida-${member.id}`);
                ...
                name: `dúvida-${member.id}`,
                type: 0, 
                parent: `${CategoriaTicket.id}`,                         topic: interaction.user.id, 
                ...
                .addFields(
                    {
                        name: '\`\`\`Suporte de Dúvidas:\`\`\`',
                        value: `*Informe sua dúvida.*`,
                        inline: false,
                    },
                )
                .setTimestamp()
