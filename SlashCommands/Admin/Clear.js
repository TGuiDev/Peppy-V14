...
module.exports = {
    ...
    run: async (client, interaction) => {
        const channelDellogs = await client.channels.cache.get(config.log_channel_id);
        ...
        if (User) {
            ...
            interaction.channel.bulkDelete(filtered, true).then(async (messages) => {
                Response.setDescription(
                    `** ${sticker.lixeira} | Deletando __${messages.size}__ mensagens de ${User.tag}**.`
                );
                interaction.reply({
                    embeds: [Response],
                });
            ...
            interaction.channel.bulkDelete(amount, true).then(async (messages) => {
                Response.setDescription(
                    `** ${sticker.lixeira} | Deletando __${messages.size}__ mensagens.**`
                );
                interaction.reply({
                    content: `${sticker.lixeira} **| Foram deletadas: __${messages.size}__ mensagens.**\n${sticker.admin} **| MOD responsvael: ${interaction.user}**`,
                });
                ...
                channelDellogs.send({
                content: `
${sticker.lixeira} **| Foram deletadas: __${messages.size}__ mensagens.**
${sticker.admin} **| MOD responsvael: ${interaction.user}**
${sticker.canal} **| Canal: ${interaction.channel}**
${sticker.discord} **| Servidor: ${interaction.guild.name}**
`,
                })
            });
        }
